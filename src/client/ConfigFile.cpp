...
void ConfigFile::parseConfigFile(DamnCute::APlayer *player_one) {
    ...
    while (std::getline(file, line)) {
        ...
        if (line.find("music_path = ") == 0) {
            saveMusicPath(line, std::string("music_path = ").size());
        }
...
void ConfigFile::saveMusicPath(std::string line, size_t pos) {
    _hasMusicPath = true;
    _musicPath = line.substr(pos, line.size() - pos);
}

bool ConfigFile::hasMusicPath() {
    return _hasMusicPath;
}

std::string ConfigFile::getMusicPath() {
    return _musicPath;
}

void ConfigFile::parseArguments(DamnCute::APlayer *player, std::string& line) {
...
void ConfigFile::saveKeys(std::string key)
{
...
void ConfigFile::saveStick(std::string stick)
{
    _hasStick = true;
    std::istringstream(stick) >> _stick;
}

void ConfigFile::saveButton(std::string button)
{
    _hasButton = true;
